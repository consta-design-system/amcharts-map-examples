...
Чтобы применить паттерн к элементу, нам нужно сделать две вещи:

- Создайть объект шаблона.
- Назначьте его элементам `fillPattern` и/или `strokePattern`.

Есть несколько видов паттернов:

- `LinePattern` - линейный узор
- `RectanglePattern` - квадратный узор
- `CirclePattern` - крговой узор
- `PicturePattern` - узор картинкой
