...
Принцип работы с группами схож с работой с точками на [карте](##LIBS.STAND/stand:amcharts-map-examples-components-points-stable).
Для начала вам необходимо создать набор элементов групп и определиться с принципом их группировки (далее будет пример группировки по областям и городам):
...
// Включение и выключение зума при прокрутке колесика мыши
...
const setGroups = (params: {
  ...
  const setupGroupStores = () => {
    ...
    // При уменьшении карты сворачиваем все группы и устонавливаем зум по умолчанию
    zoomOut?.events.on('click', () => {
      if (currentSeries) {
        currentSeries.hide();
      }
      chart.goHome();
      // Включаем зум на колесико мыши
    ...
    am5.array.each(data, (info) => {
      ...
      // Если нет групп региона создаем их
      ...
      // Если нет групп городов создаем их
  ...
  const createGroupSeries = (heatfield: string) => {
    ...
    pointSeries.bullets.push((root, series, dataItem) => {
      ...
      circle.events.on('click', function (ev) {
        ...
        // При нажатии на верхний уровень группы зум идет на регион
        if (data.type === 'state') {
          ...
          // При нажатии на нижние уровени группы зум идет на первый элемент группы
        ...
        // Отключаем зум на колесико мыши
