...
Данные для отрисовки маркера принимаются в формате `GeoJSON`, а также в виде обычных данных. Каждый набор координат отвечает за конкретную точку, до которой будет продолжаться линия
