...
Серии линий карты используются для нанесения проецируемых линий на карту.
...
Чтобы создать серию полигонов карты, нам нужно созадать ообъект `MapLineSeries` и добавить его в эементы полотна:
...
> Каждый набр координат отвечает за коонкретную точку до которой будет рисоваться линия
